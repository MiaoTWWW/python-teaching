...
# ❌ 沒有逗號 — 這不是 tuple！
not_a_tuple = (5)
print(type(not_a_tuple))  # <class 'int'> ← 只是整數！
...
# 函式回傳值的陷阱
def get_value():
    return (42)            # ⚠️ 回傳整數，不是 tuple！

result = get_value()
print(type(result))        # <class 'int'> ← 出乎意料！

def get_value_fixed():
    return (42,)           # ✅ 加上逗號才是 tuple

result = get_value_fixed()
...
# Tuple → List（可變）
rgb_tuple = (255, 128, 0)
rgb_list = list(rgb_tuple)        # [255, 128, 0]
rgb_list.append(255)              # 可修改
print(rgb_list)                   # [255, 128, 0, 255]

# List → Tuple（不可變保護）
user_list = ["Alice", "admin", "active"]
user_tuple = tuple(user_list)     # ("Alice", "admin", "active")
# user_tuple[0] = "Bob"          # ❌ TypeError：tuple 不可變
# 適用場景：傳遞給不信任的函式，保護資料不被修改
```

> 💡 **轉換時機**：需要**修改**資料時用 List，需要**保護**資料不被修改時用 Tuple，兩者之間可以隨時用 `list()` / `tuple()` 轉換。
